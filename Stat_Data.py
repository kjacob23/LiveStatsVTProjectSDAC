...
def recordData(df):
    print("Type 'back'  or 'exit' to return back to the menu at anytime ")
    def inputInitials():
    ...
    def inputActions():
        ...
        while action not in config.action_list and (action != 'BACK' and action != 'EXIT') and ("-" not in action or action[1:] not in config.action_list):
            print("There appears to be a typo please try again")
            print(action)
    ...
    while(initials and actions):
		...
		##CODE BY JACOB KAMINSKY kjacob23@vt.edu##
        if("-" in actions):
            actionWithoutSub = actions[1:]
            if (df.loc[initials, actionWithoutSub] > 0):
                if (actionWithoutSub == "FG"):
                    df.loc[initials, 'PTS']-=2
                    df.loc[initials, 'FGA']-=1
                if (actionWithoutSub == "3FG"):
                    df.loc[initials, 'PTS']-=3
                    df.loc[initials, '3FGA']-=1
                if (actionWithoutSub == "FT"):
                    df.loc[initials, 'FTA']-=1
                    df.loc[initials, 'PTS']-=1
                if (actionWithoutSub == "OREB" or actionWithoutSub == "DREB"):
                    df.loc[initials, 'TREB']+=1
                df.loc[initials, actionWithoutSub]-=1
                df.loc['Total'] = df.loc[df.index != 'Total'].sum()
            else:
                print(actionWithoutSub + " is currently at zero and cannot be subtracted from")
		##END OF CODE BY JACOB KAMINSKY kjacob23@vt##
        if(actions in config.action_list):
...
def main():
    print("Hello new user") #Can Enter Team Name Here
